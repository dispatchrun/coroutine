...
func (m *Type) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if m.CustomSerializer != 0 {
		i = encodeVarint(dAtA, i, uint64(m.CustomSerializer))
		i--
		dAtA[i] = 0x60
	}
	if m.ChanDir != 0 {
...
func (m *Type) SizeVT() (n int) {
	...
	if m.CustomSerializer != 0 {
		n += 1 + sov(uint64(m.CustomSerializer))
	}
	if m.Variadic {
		n += 2
	}
	...
	for iNdEx < l {
		...
		case 12:
			...
			for shift := uint(0); ; shift += 7 {
				...
				b := dAtA[iNdEx]
				iNdEx++
				m.CustomSerializer |= uint32(b&0x7F) << shift
				if b < 0x80 {
					break
				}
			}
